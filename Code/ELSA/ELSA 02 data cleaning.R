...
for(i in 5:46){pain[,i]<-as.numeric(as.character(pain[,i]))}
summary(pain)
...
ELSA_pain_imputed <- mice(pain, m=1, maxit = 5, method = 'cart', seed = 1)

#Extract the data
ELSA_pain_imputed <- complete(ELSA_pain_imputed)
write.csv(ELSA_pain_imputed, "pain_elsa_imputed.csv")