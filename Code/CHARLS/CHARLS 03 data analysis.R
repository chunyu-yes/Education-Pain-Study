...
CHARLS_pain_imputed <- read.csv("pain_charls_imputed.csv")
...
t1_unweighted = CreateTableOne(vars = c("age", "agegroup", "sex", "high_edu", "high_edu_parent", "height",
                                        "hukou", "liverural", "pain_status", "pain_med"), 
...
charlsSvy <- svydesign(ids = ~ ID, weights = ~ weight, nest = F, data = CHARLS_pain_imputed)
...
# fit<-glm(pain_interfere ~ female + edu_rank + agegroup + liverural  + height + high_edu_parent + hukou, family=binomial(link = "probit"), weights=weight, data=subset(CHARLS_pain_imputed, pain_status == 1))
# 
# summary(fit)
# tidy<-as.data.frame(summary(fit)$coefficients[2:3,])
# tidy$label<-'pain_interfere'
