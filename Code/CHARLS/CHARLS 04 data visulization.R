...
CHARLS_inter <- glm(pain_status ~ high_edu*female + age + high_edu_parent + liverural + hukou + height, family=binomial(link = "probit"), weights=weight, data=CHARLS_pain_imputed)
...
ggplot(CHARLS.pain_adjusted, aes(as.factor(high_edu), lsmean, color=as.factor(female))) +        
...
hrs_q1 + hrs_q2 + hrs_q4 + hrs_q5 + 
mhas_q1 + mhas_q2 + mhas_q4 + mhas_q5 + 
charls_q1 + charls_q2 + charls_q4 + charls_q5 + 
lasi_q1 + lasi_q2 +   
plot_layout(ncol = 4)