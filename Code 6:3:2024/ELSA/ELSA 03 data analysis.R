...
ELSA_pain_imputed<-read.csv("pain_elsa_retired_imputed.csv")
...
ELSA_pain_imputed[, c(7,40,49,51,18,37,38,47)] <- lapply(ELSA_pain_imputed[, c(7,40,49,51,18,37,38,47)], as.factor)
...
"pain_status","pain_level","retired"), 
							   data=ELSA_pain_imputed, includeNA = TRUE)
...
charlsSvy <- svydesign(ids = ~ ID, weights = ~ weight, nest = FALSE, data = ELSA_pain_imputed)
...
fit<-glm(pain_status ~ female + high_edu, family=binomial(link = "probit"), weights=weight, data=ELSA_pain_imputed)
...
fit<-glm(pain_level ~ female + high_edu, family=binomial(link = "probit"), weights=weight, data=ELSA_pain_imputed)
...
fit<-glm(pain_status ~ female + high_edu + agegroup + height + high_edu_parent + race, family=binomial(link = "probit"), weights=weight, data=subset(ELSA_pain_imputed,retired == 1))
...
fit<-glm(pain_level ~ female + high_edu + agegroup   + height + high_edu_parent + race, family=binomial(link = "probit"), weights=weight, data=subset(ELSA_pain_imputed, pain_status == 1 & retired == 1))
...
fit<-glm(pain_status ~ female + high_edu + agegroup   + height + high_edu_parent + race, family=binomial(link = "probit"), weights=weight, data=subset(ELSA_pain_imputed,retired == 0))
...
fit<-glm(pain_level ~ female + high_edu + agegroup   + height + high_edu_parent + race, family=binomial(link = "probit"), weights=weight, data=subset(ELSA_pain_imputed, pain_status == 1 & retired == 0))
