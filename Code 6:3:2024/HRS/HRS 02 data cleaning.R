...
# read in the data
pain_hrs <- read.csv("pain_hrs_retired.csv")
pain <- pain_hrs[, -1]
colnames(pain)
...
HRS_pain_imputed <- mice(pain, m=5, maxit = 10, method = 'cart', seed = 1)

#Extract the data
HRS_pain_imputed <- complete(HRS_pain_imputed)

write.csv(HRS_pain_imputed, "pain_HRS_retired_imputed.csv")
