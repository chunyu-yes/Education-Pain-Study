...
# read in the data
pain <- read.csv("pain_charls_retired.csv")
pain <- pain[, -1]
colnames(pain)
...
#retired
table(pain$retired , exclude = NULL)
...
CHARLS_pain_imputed <- mice(pain, m=5, maxit = 10, method = 'cart', seed = 1)

#Extract the data
CHARLS_pain_imputed <- complete(CHARLS_pain_imputed)

write.csv(CHARLS_pain_imputed, "pain_charls_retired_imputed.csv")
